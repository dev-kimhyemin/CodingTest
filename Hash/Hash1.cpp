...
// 데이터를 키로서 정렬하지 않고 사용하는 것으로 충분하므로 unordered_set를 채택함
int solution(vector<int> nums)
{
    const unordered_set<int> ownNumbers(nums.begin(), nums.end());

    const int ownNumbersSize = static_cast<int>(ownNumbers.size());
    const int select = static_cast<int>(nums.size()) / 2;

    return ownNumbersSize >= select ? select : ownNumbersSize;
}
